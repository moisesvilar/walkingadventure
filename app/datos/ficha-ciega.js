// La primitiva de la ficha ciega en el móvil: cegar, esperar la firma, desciegar. Es
// **la copia declarada** de `server/ficha-ciega.mjs`, y de ahí abajo las dos son el mismo
// texto, función por función y línea por línea.
//
// Que sea una copia y no un import no es pereza y conviene tener el motivo delante,
// porque el reflejo correcto al leer esto es querer arreglarlo:
//
// - `app/` **no puede importar por ruta relativa fuera de su directorio** —lo afirman
//   `test/nucleo/app.test.mjs` y `test/nucleo/paquete.test.mjs`—, así que
//   `../../server/ficha-ciega.mjs` no es una opción.
// - Alcanzarlo por nombre exigiría un paquete nuevo del espacio de trabajo, y la lista
//   cerrada de dependencias de la app no admite ninguno que ninguna spec nombre.
// - Meterlo en `packages/nucleo/` sería meter criptografía del proxy en el generador
//   determinista del mundo, que es otra cosa.
//
// Es el mismo trato que `datos/consulta-osm.js` tiene con `server/aguas-arriba/overpass.mjs`
// y por el mismo motivo: aquel módulo abre con `node:crypto` y el empaquetador del móvil
// no lo resuelve. La diferencia, y es a favor de esta copia: si las dos divergen, aquí no
// se pierde una caché en silencio — **no verifica ni una sola ficha**, y se ve en la
// primera generación de mapa.
//
// Regla, entonces: se tocan las dos o no se toca ninguna, y `VERSION_FICHA_CIEGA` sube en
// las dos a la vez. Un `diff` de este fichero contra el del servidor a partir de la línea
// del SHA-256 tiene que salir vacío.

/**
 * La versión de la primitiva. Copia declarada de la del servidor: subirla allí obliga a
 * subirla aquí, y dos números distintos son la señal de que alguien tocó una mitad.
 */

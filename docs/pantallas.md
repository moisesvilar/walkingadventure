...
**Y desde la fila 53, la pantalla 7 dibuja dos gestos donde antes dibujaba uno encadenado.** «Guardar una copia primero» y «Borrar sin guardar nada» eran los dos extremos de la misma cadena: guardar llevaba al borrado y del borrado al arranque. Ahora **guardar solo guarda** —la pantalla se queda donde está, con la línea «La copia ya está hecha. Tu partida sigue como estaba.» y las tres acciones puestas— y **borrar es un segundo toque**, el único camino al arranque. El botón destructivo pasa a llamarse «Borrar la partida», porque con una copia recién hecha delante «sin guardar nada» es falso. Lo cierra un principio que no es de esta pantalla: **lo destructivo no se ejecuta sobre una señal que el sistema no garantiza**. Lo trajo una medida en el aparato — la hoja de compartir de Android se da por resuelta en cuanto se abre, así que la partida se borraba mientras quien juega elegía dónde guardarla, 25 s con la hoja en pantalla y los documentos ya vacíos. La jerarquía no se mueve: guardar sigue arriba y sólida, borrar sigue hueca y en rojo, dejarlo como está sigue sin desaparecer nunca. Manda `docs/specs/SPEC-040-iter-1-empezar-de-nuevo.md`, y `docs/flujo.md` recoge la arista nueva. Iteración: `docs/starting.md`, 13-ago-2026.

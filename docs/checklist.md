...
| 54 | cuaderno-de-a-bordo | RF-INFRA-007 | should | done (cotejo independiente 14-ago en worktree, a la segunda: el primero cazó `empezar-de-nuevo-copia` intermitente — misma foto, una tanda PASS y otra FAIL — y la iteración lo cerró esperando `android:id/resolver_list` antes del `back`, 18/18; final 2975 · 2972 · 0 · 3 y @app 22 · 13 · 0 · 9, PASS EXIT=0 — ver §18) |
| 55 | rotulo-vivo-ios | RF-BUCLE-001, RF-INFRA-004, RF-PRIV-002 (mitad iOS) | must | pending |
| 56 | fuente-de-salud-ios | RF-RUMOR-002, RF-RUMOR-006, RF-PRIV-003 (mitad iOS) | should | pending |
...
La **54** nace del salto a iPhone que el dueño quiere ya (`docs/iphone.md`): en su aparato no habrá Metro y los `console.log` van a Metro, así que de pie no hay dónde mirar. El cuaderno de a bordo — interruptor de logs exhaustivos en la puerta de desarrollo más botón de compartir por la hoja del sistema — se construye y prueba en `wa-pixel` **antes** del salto. Tras `__DEV__`, fuera de `flujo.md` (doctrina §6y), y con la tensión con la regla de la puerta («no escribe nada») resuelta declarando, no silenciando. El encargo completo está en `docs/prompts/prompt-cuaderno-de-a-bordo.md`.

La **55** y la **56** nacen del estreno de la app en el iPhone del dueño (14-ago-2026, `docs/feedback-testing-14-ago.md` §3): la pantalla de capacidades dijo las dos verdades que las filas 46 y 48 dejaron escritas — sin widget de ActivityKit no hay rótulo y **en iOS una salida no se abre**; sin HealthKit no hay de dónde leer los pasos — y el dueño dictó que se resuelven antes de su primer paseo real. Las dos con dependencia ratificada el mismo día: **módulos nativos propios, en el repo** (Swift como módulos Expo locales, plugins propios en la guarda de lista cerrada), descartadas librerías externas. Primero la 55, que es la que bloquea la salida; la 56 no se lanza hasta fusionarla. Los encargos completos están en `docs/prompts/prompt-rotulo-vivo-ios.md` y `docs/prompts/prompt-fuente-de-salud-ios.md`.

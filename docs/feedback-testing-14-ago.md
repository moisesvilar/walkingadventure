...
## 7 · La primera cosecha del cuaderno de a bordo — dos hallazgos y un hueco

El dueño compartió el JSONL por el circuito previsto (hoja del sistema → WhatsApp → chat) tras el incidente de la entrada 6. 450 eventos, y de su lectura salen tres cosas:

1. **La primera captura real de la familia de la carrera de la salida sorda** (SPEC-048, fichada sin medida hasta hoy): a las 14:05:19Z, con la salida «Comarcas da Lúa Rota» abierta con rótulo, llegó una posición con marca **1786716312681 ms, anterior en ~7 s a la última recibida (1786716319736 ms)**; la guarda la rechazó con su motivo exacto —«tratarla como reciente reiniciaría el plazo hacia atrás»— y la marca `salida-averia` pasó de `sin-averia` al mensaje, con pila. **Core Location entrega fijos desordenados** (el primer fijo cacheado puede llegar detrás de uno fresco): es la medida que a la carrera le faltaba, y es de iOS. La salida siguió viva y se cerró desde el rótulo a las 14:08Z.
2. **143 eventos de re-anclaje en una salida de ~3 minutos** — uno por posición, cuando el re-anclaje se diseñó como gesto excepcional (el primer fijo que repara de verdad, fila 53). Posiblemente alimentado por los fijos desordenados del punto 1. Sin diagnóstico: es dato para la fila que lo recoja, no conclusión.
3. **Un hueco de cobertura del propio cuaderno**: el fallo de la entrada 6 —«tu partida guardada no se ha podido abrir»— **no dejó ni un evento**. El cuaderno graba la vida de la salida y los errores globales, pero el camino de la partida que no abre no emite nada: la evidencia fue el pantallazo del dueño, no el log. Para la próxima avería de apertura, la linterna está ciega — fichar junto al cuaderno (fila 54, dueño del instrumento).

El JSONL queda fuera del repo a propósito (lleva las coordenadas reales del dueño; el trato de privacidad de la fila 54 es que salen solo por su mano y a quien él elija); las líneas que importan están citadas aquí con sus milisegundos.

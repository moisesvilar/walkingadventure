...
**Y la contrapartida del aparato limpio, medida por el cotejo de la 50: el proveedor de ubicación frío no da posición puntual.** Desde aparato limpio, `en-marcha.yaml` y `telon.yaml` caen al abrir la salida con «Current location is unavailable» (el error de `getCurrentPositionAsync`), aunque el bucle de `geo fix` alimente cada 2 s, `dumpsys location` enseñe fijo fresco en `gps` y `fused`, **y el arranque de la misma tanda ancle el mapa en ese mismo fijo**. No es regresión — el mismo código pasó esos dos flujos la mañana del 12-ago con el emulador caliente, y el rojo se reproduce sobre main sin el merge de la 50 — y **reiniciar el emulador no lo cura** (medido: reinicio + túneles + bucle de posición, mismos dos rojos con el mismo motivo). El verde de esos dos flujos depende de que el fused provider esté caliente, y eso hoy no lo gobierna ni el flujo ni el runner; la decisión de producto que lo cerraría (¿cae la apertura a la última posición conocida fresca?) está en `docs/pendientes.md`.

...
Desde SPEC-020 hay un `package.json` en la raíz, y su único cometido es declarar el espacio de trabajo (`app` y `packages/*`) para que la app resuelva el núcleo por su nombre, `@walkingadventure/nucleo`. **No arrastra nada más**: `packages/nucleo/` sigue con cero dependencias, el prototipo sigue siendo JavaScript ESM puro sobre navegador con Leaflet y las fuentes por CDN en `prototipo/index.html`, y `node test/headless.mjs` y `node --test test/nucleo/` **tienen que seguir arrancando en un clon limpio sin instalar nada**. El día que la red de seguridad del determinismo dependa de un `node_modules`, deja de ser una red. Las dependencias de la app son las que nombra su spec y ninguna más; no introduzcas toolchain sin que el usuario lo pida.

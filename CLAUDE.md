...
**El runner toma el aparato por presencia, no por declaración.** `bash scripts/qa-tester-run.sh <ETIQUETA>` corre también `@app` si encuentra Maestro y un emulador conectado — decide `command -v maestro` más `adb devices`, y ninguna declaración de recursos lo frena. **Una tanda sin aparato se invoca con `--nucleo-only`**, que el runner ya trae. Medido por la fila 51, cuya invocación cómoda habría tomado un emulador que tenía prohibido; `wa-qa-tester` lo cazó comprobando el terreno antes de arrancar. Es la familia de la tubería: la invocación cómoda hace algo distinto de lo que crees. Y otra de la misma talla: **`timeout` no existe en este macOS** — el comando falla con `command not found`, el log queda de una línea y el `exit` que lees es el del fallo, no el de la medida.
...
**Los rojos de `@app` solo son comparables desde un aparato limpio, y limpiarlo es un paso explícito.** Medido por la fila 50: la misma `main` que declara 2 rojos fichados dio **7** contra un emulador que arrastraba un arranque a medias y un mapa levantado de otra sesión — cinco flujos que asumen instalación limpia caen por estado, no por regresión, y se leen como avería. Antes de cualquier tanda cuyos números vayan a compararse: `adb shell pm clear com.walkingadventure.app`, y después reinstalar. No basta con `force-stop`, y `expo run:android` **no** borra los documentos. La precondición se declara junto a los números o los números no dicen nada.

**Lo que durante dos días se llamó «el proveedor de ubicación frío» no era el proveedor: la puntual pedía con precisión equilibrada, y con esa el sistema no enciende el GPS.** Medido el 13-ago-2026 por la fila 53, y corrige lo que aquí ponía. El síntoma era real —`en-marcha.yaml` y `telon.yaml` cayendo al abrir la salida con «Current location is unavailable», 30-32 s de espera para un «no», reproducible desde aparato limpio y sin curarse reiniciando el emulador—, y la raíz es una línea: `posicionPuntual()` pedía con `Accuracy.Balanced` mientras la suscripción pedía con `Accuracy.High`. Durante esos 30-32 s el Event Log de `dumpsys location` **no registró ni una petición**; la suscripción sí registra la suya (`ProviderRequest[@+2s0ms, HIGH_ACCURACY]`) en cuanto la salida se abre. Con precisión alta el mismo aparato entrega en **5,0 s**, medido en tres tomas con el GPS frío. Lo que hay que llevarse de aquí no es el arreglo: **la causa llevaba desde el 11-ago escrita en el propio módulo, veinte líneas por encima del defecto** (`app/plataforma/posiciones.js`, el comentario de la precisión), y aun así **dos cotejos independientes** lo atribuyeron al proveedor frío. Los dos leyeron bien lo que veían; lo que veían no era lo que había — en parte por la trampa de abajo, que hacía que `dumpsys location` pareciera decir que el fijo estaba fresco y a mano. La defensa que quedó puesta es de forma y no de disciplina: las dos peticiones leen la misma función, así que no se pueden volver a separar sin escribirlo aposta.

**`adb emu geo fix` no inyecta nada si nadie pide posición.** Medido el 13-ago-2026: con el bucle corriendo cada 2 s durante cuatro minutos, `dumpsys location` seguía enseñando el fijo persistido del arranque del emulador y el Event Log **no tenía un solo evento desde el boot**; en cuanto otra app pidió posición apareció el fijo del bucle con **0,6 s** de edad. El GPS del emulador es **bajo demanda**, y de ahí dos corolarios: un `dumpsys location` con fijo viejo **no prueba que el bucle no esté alimentando** —prueba que nadie ha pedido—, y toda medida de apertura tiene que declarar si el bucle estaba corriendo en lugar de deducirlo de lo que `dumpsys` enseñaba antes. Es la misma familia que el `pm clear`: la precondición se declara junto a los números o los números no dicen nada.

**Sin Metro delante, la APK de depuración no lleva el bundle dentro, y el rojo que produce es indistinguible de una regresión.** Todos los flujos `@app` caen en el primer `assertVisible` con «Unable to load script» — no hay pantalla, no hay pista en el report, y lo que se lee es que la app está rota. Y con Metro pero sin `EXPO_PUBLIC_PROXY`, la app monta y enseña `arranque-sin-cablear` («el cliente de atestación necesita la dirección del proxy, y no la adivina»): otro rojo, distinto y también silencioso. La receta buena es `EXPO_PUBLIC_PROXY=http://10.0.2.2:8138 npx expo start` desde `app/`, con `server/arranca.mjs` vivo en el 8138 y los `adb reverse` de 8081, 8137 y 8138. **Ninguna de las dos precondiciones la declara `test/app/arranque.yaml`**, así que hoy solo viven aquí.

**El volcado del estado a disco es perezoso.** `files/partida/partida/estado.json` solo se refresca cuando la app pasa a fondo, así que leerlo con la app en primer plano enseña una instantánea vieja. Ya produjo una lectura engañosa —`seAlejo: False` en el fichero cuando en memoria ya era `True`—, y la defensa es mandar la app a fondo antes de leer.
...
**Una capa que se declara con `...StyleSheet.absoluteFillObject` puede no posicionarse.** Medido por la fila 50 en `wa-pixel`: la capa del descarte con el spread salía en `[0,2122][1080,2400]` —apilada debajo de la pantalla que debía cubrir, con su contenido sin pintar— y con `position: 'absolute'` más las cuatro anclas escritas una a una sale `[0,0][1080,2400]`. Escríbelas explícitas. Y ojo con lo que se afirma: una prueba que exija el spread da por bueno exactamente el estado que no funciona. `app/pantallas/visor.js` sigue usando el spread y **no se ha medido**.

**Un total de núcleo que baja no es una tanda mejor.** Medido por la fila 46: sin compilar la app no hay manifiesto fusionado, así que el bloque que lo mira **no se registra** y la batería pasa de 2825 a 2819 casos — y con esos seis se va el rojo ajeno de `BOOT_COMPLETED`, que entonces **no está verde: está sin mirar**. La comprobación es `test/reports/manifiesto-generado.estado.json`, que tiene que decir `mirado: true` en las dos plataformas y `completo: true`; un report que no lo diga no es comparable con ninguno que sí. Es la misma familia que la tubería que se traga el código de salida: el número sale limpio y lo que ha desaparecido es la medición.

**Y su hermana, medida por la fila 52: `expo run:android` sobre un `app/android/` que ya existe no vuelve a correr prebuild.** Un cambio en un config plugin no llega entonces al manifiesto fuente, `processDebugMainManifest` sale `UP-TO-DATE` y **lo que la guarda mide es el manifiesto de la tanda anterior**: el `mirado: true` es cierto y el artefacto es viejo. Hace falta `cd app && npx expo prebuild --platform android --no-install --skip-dependency-update expo` **antes** de compilar. Ojo con que `test/reports/manifiesto-generado.estado.json` declara solo `expo run:android` como receta de Android: sirve para regenerar el APK, no para propagar un cambio de plugin. En un worktree recién creado no aplica —ahí no hay `app/android/` y el prebuild corre solo—, que es justo por qué la trampa solo aparece en el árbol principal. Y dos efectos del mismo comando que hay que contar: **instala y abre la app en el emulador** (así que toma el aparato aunque solo querías compilar, y quien mida `@app` después necesita su `pm clear`), y **necesita `ANDROID_HOME=$HOME/Library/Android/sdk` en el entorno** o falla en cuatro segundos con «SDK location not found».

**Cuidado con dónde tiene un control su área sensible, y con qué se puede leer de él.** Dos medidas de la fila 46 sobre la fila de un interruptor de A6P6, y son distintas. La primera: el `Switch` ocupaba `[906,1428][1028,1499]` dentro de una fila de `[53,1397][1028,1532]`, así que **un toque al centro caía en la etiqueta y no llegaba al control** — no había ni encendido ni aviso, que son las dos únicas salidas del código, porque no llegó a correr. Se arregla haciendo responder a la fila entera y dejando el control sordo (`pointerEvents="none"`), que además hace imposible por construcción que lo pintado se mueva sin que se mueva el valor. La segunda: **`checked:` sobre esa fila no discrimina en ninguna dirección** — el `ViewGroup` dice `checked=false` también con el interruptor encendido y el `Switch` es un nodo hermano sin identificador, así que una aserción sobre él pasa siempre y no mide nada. Llevaba así desde la fila 43. Y una precisión sobre la trampa de las cotas de arriba: **vale para `adb input tap` y no para Maestro**, cuyo `tapOn` sí acierta el control.

**Un flujo de `@app` puede dejar el aparato tocado para los que vienen detrás.** Medido por la fila 46: `ajustes.yaml` pasa en verde y deja concedidos los dos permisos de salud, que ningún flujo posterior puede devolver desde dentro de la app — un permiso concedido no se revoca solo. Y hay un segundo, medido en las dos direcciones y **sin explicar**: `arranque.yaml` deja `READ_DISTANCE` y `READ_STEPS` en `granted=true` aunque se neutralice la llamada de la app a `requestPermission`. Corolario para cualquier medición sobre esos permisos en este emulador: **su valor no prueba que la app los haya pedido**, y la defensa es la de siempre, `pm clear` antes de medir.

...
// La usamos para: no colocar núcleos en el mar, pintar el mar, y calcular el radio dinámico.
...
function segSideDist(p, a, b) {
  ...
  // extremos el signo del cross respecto a la recta infinita puede mentir.
  const interior = tRaw > 0.02 && tRaw < 0.98;
  return { d, sea: cross < 0, interior };
}

/**
 * coastlines: array de polilíneas [{x,y},...] en metros.
 * extent: semi-lado del cuadrado cubierto (metros). cell: tamaño de celda (metros).
 *
 * Cada celda se clasifica por el LADO del segmento de costa más cercano
 * (búsqueda con índice espacial por cubos). Es robusto frente a huecos en los
 * ways de costa, al contrario que un relleno por inundación.
 */
export function buildSeaMask(coastlines, extent, cell = 200) {
  ...
  // índice espacial: cubos de 1 km con los segmentos cuyo bbox los toca
  const bucket = 1000;
        ...
        for (let bj = Math.max(0, cbj - r); bj <= Math.min(bn - 1, cbj + r); bj++) {
          for (let bi = Math.max(0, cbi - r); bi <= Math.min(bn - 1, cbi + r); bi++) {
            ...
            for (const si of buckets[bj * bn + bi]) {
              ...
              // penaliza los extremos de segmento: a igualdad práctica de
              // distancia gana un segmento con proyección interior (lado fiable)
...
/**
 * Radio de visualización dinámico: amplía desde rBase hasta que el borde del
 * círculo solo corte agua de océano abierto (inevitable), nunca bahías/rías,
 * y añade un margen extra. Devuelve rBase si no hace falta ampliar.
 */
export function computeDisplayRadius(mask, { rBase = 20000, rMax = 26000, rOceanTest = 27500, margin = 4000, step = 500, samples = 720 } = {}) {
  ...
  if (enclosedCount(rBase) === 0) return rBase; // el borde ya solo toca tierra u océano

...
export function renderMap(canvas, world, view = null) {
  ...
  const hits = [];
  drawParajes(ctx, world.parajes ?? [], px, v.paraje, hits);
  drawSettlements(ctx, world.settlements, px, v.focus, hits);
  if (v.focus) drawServiceMarkers(ctx, v.focus, px);
  if (v.quest) drawQuestOverlay(ctx, v.quest, px);

  ctx.restore();

  drawBorder(ctx, cx, cy, R);
  drawCompass(ctx, cx, cy, R);
  drawCartouche(ctx, W, v.focus ? v.focus.name : v.paraje ? v.paraje.name : world.title);
...
function drawParajes(ctx, parajes, px, focusParaje, hits) {
  for (const p of parajes) {
    const q = px(p);
    drawParajeGlyph(ctx, p.type, q, p === focusParaje);
    drawTextLabel(ctx, p.name, q.x, q.y + 12, 'italic 13px "IM Fell English", Georgia');
    hits.push({ x: q.x, y: q.y, r: 16, paraje: p });
  }
}

function drawParajeGlyph(ctx, type, q, highlight) {
  ctx.save();
...
function drawQuestOverlay(ctx, quest, px) {
  ...
  ctx.lineJoin = 'round';
  ctx.setLineDash([9, 7]);
  ctx.strokeStyle = 'rgba(122,46,29,0.85)';
  ctx.lineWidth = 3;
  ctx.beginPath();
...
function drawServiceMarkers(ctx, settlement, px) {
  for (const p of settlement.services) {
    ...
    drawTextLabel(ctx, p.name, q.x, q.y + 6, 'italic 18px "IM Fell English", Georgia');
  }
}

function drawTextLabel(ctx, text, x, y, font) {
  ctx.font = font;
  ctx.textAlign = 'center';
...
// Costa OSM: agua a la derecha del sentido de dibujo → olas hacia ese lado.
...
function drawSettlements(ctx, settlements, px, focus, hits) {
...
function drawLabel(ctx, s, q, size) {
  ...
  drawTextLabel(ctx, s.name, q.x, q.y + size * 0.75 + 3, LABEL_FONT[s.type]);
}

...
## §18 · La fila 54: el cotejo que existió para su día (14-ago-2026)

### 18a · El flojo que una tanda no ve

El cierre de la fila 54 (el cuaderno de a bordo) llegó con SUITE PASS declarada, y el cotejo independiente sobre su HEAD `005dcb5` dio **FAIL**: `empezar-de-nuevo-copia.yaml` en rojo — **el mismo hash, una tanda verde y otra roja**. Es la primera vez que el cotejo caza exactamente aquello para lo que se instituyó en §13: no un error de la fila, sino una intermitencia que ninguna tanda única puede descartar. La evidencia no fue el log sino **la captura de Maestro del instante del fallo** (`step-106`): la hoja de compartir del sistema plantada encima y, debajo, Ajustes en vez de la pantalla esperada. Lectura mecánica: bajo carga de suite, el `back` llegaba antes de que el chooser terminara de subir, la app se lo comía (retrocedía de pantalla) y la hoja aparecía después y se quedaba. **La carrera estaba antes del `back`**, y el parche de última hora de la fila (`extendedWaitUntil` de 20 s *después*) sincronizaba el lado equivocado. Las medidas de acotación: aislado con `pm clear`, verde; pareja cuaderno→copia, verde; solo caía con la suite cargando el emulador. La iteración lo cerró por la raíz —esperar `android:id/resolver_list`, el elemento estable del selector nativo, antes de cancelar— y lo probó al listón de la 53: **18/18 con `pm clear` antes de cada una**, más SUITE PASS. El cotejo de verificación reprodujo: **2975 · 2972 · 0 · 3, @app 22 · 13 · 0 · 9, EXIT=0**. Segunda fila consecutiva con el proyecto entero en verde.

### 18b · El hash declarado no puede quedarse detrás del HEAD

La bitácora de la fila citaba `a4f84c8` como cierre y su rama tenía un commit de test posterior (`005dcb5`) sin tanda declarada — el hash fichado se había quedado atrás de lo que se entregaba. De la resolución salió una convención que queda para todas las filas: **la bitácora cita literal el último hash ejecutable probado, y encima va como mucho un único commit exclusivamente documental**, declarado como tal en la propia entrada y verificable con `git diff <hash> HEAD --stat` (un solo fichero fuera de `docs/` lo invalida). La alternativa simbólica («el commit que contiene esta entrada») se descartó: pierde el hash literal de la auditoría y obliga a bendecir documentación con suites de 40 minutos. La imposibilidad de que un commit contenga su propio SHA no era el problema; el problema era que **lo ejecutable se movió después de la última medida**.

### 18c · La mensajería entre pares tiene un modo de fallo silencioso

El veredicto del cotejo se envió por mensajería a la sesión de la fila y quedó **retenido en un socket cuya ventana ya no existía**: el dueño no vio nada y el mensaje no llegó jamás. El circuito robusto siempre fue el otro: **el encargo autocontenido que el dueño pega** — el mensaje puede acompañar, nunca sostener. Misma familia que §14 (el recurso con dueño): un canal solo cuenta como canal si alguien está mirando el otro extremo. Y del cotejo mismo, un apunte de método operativo: el agente de cotejo en segundo plano soltó el turno tres veces al quedarse esperando procesos `nohup`; la forma que funcionó fue **el vigía propio del orquestador sobre el log** (`until grep EXIT=`), despertando al agente en cada hito. El que espera sin vigilar no espera: duerme.

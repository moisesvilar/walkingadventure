...
3. **Al parar en un lugar** — pantalla permitida, porque estás quieto y has validado la llegada. Aquí ocurre la revelación del anclaje, la escena, y lo que allí se cuenta. **Validar la llegada no es un gesto ni un disparo** (6-ago-2026): al detectar que estás parada dentro del geofence, la escena queda **disponible** y espera. No enciende la pantalla, no pone la app en primer plano, no te llama. Si miras, está ahí; si no, sigues andando y no ha pasado nada, que es lo que hace que pararse en un semáforo no tenga consecuencias. Se descartó abrirla sola, que no se pierde nadie nada pero convierte el móvil en algo que te llama en vez de avisarte. **Y cómo se detecta que estás parada dentro no es un detalle de implementación** (11-ago-2026, con la medida delante): se compara el centroide de la primera mitad de una ventana de posiciones con el de la segunda, y no dos fijos seguidos, porque el ruido del GPS es de media cero y la deriva de quien anda no; promediar media ventana hunde el ruido y deja la deriva intacta. Medir de fijo a fijo parecía suficiente y no lo era: con diez metros de error, alguien quieto cinco minutos dentro del geofence validaba el 13 % de las veces, y con la cadencia del sensor que había en el teléfono, ninguna. La ventana es de **veinte segundos** cuando el error que declara el fijo la sostiene y de **cuarenta** cuando no, de modo que la permanencia corta —que es la que hace que un beat atendido de paso valide igual— se conserva donde se puede sostener y solo se estira donde el dato es malo. **Y tiene un límite, que se dice con número y no con esperanza**: por encima de σ ≈ 15 m la validación se degrada y por encima de σ ≈ 20 m deja de sostenerse, así que esto cubre la calle normal y no cubre el cañón urbano profundo. El vehículo sigue apartándose por su cuenta y no por la deriva: un coche parado en un atasco no deriva, así que si la quietud fuese el único criterio, el autobús validaría la escena desde la ventanilla.
...
**El punto de partida cuenta como sitio para la cadencia del sensor, y solo para eso** (13-ago-2026). Estando quieta en él el muestreo pasa a ser por tiempo, igual que dentro de cualquier sitio del mapa. El porqué está medido y es incómodo: con la cadencia por distancia, quien está parada **no recibe ni un fijo** —cero en 5 min 56 s, el primero a los 355,8 s y solo al mover la posición—, y sin fijos la permanencia del regreso no acumula nunca. O sea que el telón que este apartado declara como la norma **no podía ocurrir**: no era que cayera tarde o de vez en cuando, es que era estructuralmente imposible volver a casa y que se cerrara la salida. Y la contrapartida, que es lo que impide que esto se cuele en el juego por la puerta de atrás: el punto de partida **no es un sitio**. No valida ninguna llegada, no consume anclaje, no tiene escena ni ficha y no se pinta como sitio en el mapa; entra a la decisión de la cadencia y a nada más. El portal de casa no es un lugar al que se llega.

**La apertura de una salida no depende de que el cielo responda** (13-ago-2026). Se pide la posición con precisión alta y con un tope de espera declarado, se dice en pantalla que se está buscando —sin cifras, sin barra y sin cuenta atrás, que sería prometer lo que el sistema no garantiza—, y si el fijo no llega se acepta la última posición conocida **dentro de la misma cota de frescura** que se le exige a cualquier otra: lo que ancla el punto de partida es un fijo, no la puerta por la que entró. El ancla se corrige una sola vez, con el primer fijo que la repare de verdad y mientras todavía no se haya andado nada; después de alejarse ya es inmutable, porque mover «casa» a mitad de salida cambia el sitio al que hay que volver bajo los pies de quien vuelve. Y si no hay nada dentro de la cota se dice la verdad y la salida no se abre. La razón de fondo es la de todo este apartado: **el telón lo echa volver**, así que sin punto de partida no hay bucle — y una salida que no llega a abrirse por un fijo que tardaba cinco segundos deja el bucle sin empezar, exactamente igual de roto que una que se abre con un punto viejo y lo deja sin cerrar.
